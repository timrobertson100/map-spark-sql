...
// https://raw.githubusercontent.com/LinkedInAttic/camus/master/camus-sweeper/src/main/java/com/linkedin/camus/sweeper/mapreduce/AvroKeyCombineFileInputFormat.java
public class AvroKeyCombineFileInputFormat extends CombineFileInputFormat<AvroKey, NullWritable> {
  ...
  public List<InputSplit> getSplits(JobContext arg0) throws IOException {
    ...
    for (int i = 0; i < splits.size(); i++) {
      ...
      if (locations.length > 10) locations = Arrays.copyOf(locations, 10);

      cleanedSplits.add(
          new CombineFileSplit(
              oldSplit.getPaths(), oldSplit.getStartOffsets(), oldSplit.getLengths(), locations));
    }
  ...
  private void addAllAvro(List<Path> files, List<Path> dirs, FileSystem fs)
      throws FileNotFoundException, IOException {
    for (Path dir : dirs) {
      for (FileStatus f : fs.listStatus(dir)) {
        if (f.isDir()) addAllAvro(files, Collections.singletonList(f.getPath()), fs);
        else if (f.getPath().getName().endsWith(".avro")) files.add(f.getPath());
      }
    }
  }

  @Override
  public RecordReader<AvroKey, NullWritable> createRecordReader(
      InputSplit arg0, TaskAttemptContext arg1) throws IOException {
    return new CombineFileRecordReader<AvroKey, NullWritable>(
        (CombineFileSplit) arg0, arg1, AvroKeyCombineFileReader.class);
  }

  public static class AvroKeyCombineFileReader extends RecordReader<AvroKey, NullWritable> {
    AvroKeyRecordReader<GenericRecord> innerReader;
    CombineFileSplit split;
    TaskAttemptContext context;
    Integer idx;

    public AvroKeyCombineFileReader(CombineFileSplit split, TaskAttemptContext arg1, Integer idx)
    ...
    public void initialize(InputSplit arg0, TaskAttemptContext arg1)
        throws IOException, InterruptedException {
      innerReader.initialize(
          new FileSplit(split.getPath(idx), split.getOffset(idx), split.getLength(idx), null),
          arg1);
    }
